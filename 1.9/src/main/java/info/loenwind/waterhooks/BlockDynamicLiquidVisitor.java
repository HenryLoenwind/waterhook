...
public class BlockDynamicLiquidVisitor extends ClassVisitor implements IClassTransformer {
  ...
  private final String description = "(Lnet/minecraft/world/World;Lnet/minecraft/util/math/BlockPos;Lnet/minecraft/block/state/IBlockState;Ljava/util/Random;)V";
	...
	private static class HookMethodVisitor extends MethodVisitor {
      ...
      7->8 mv.visitJumpInsn(IF_ACMPNE, l23);
        ...
        if (opcode == Opcodes.GETSTATIC && "net/minecraft/block/material/Material".equals(owner)
            && ("field_151586_h".equals(name) || "water".equals(name) || "WATER".equals(name))) {
					state ++;
				} else {
					state = 0;
				}
		...
		public void visitJumpInsn(int opcode, Label label) {
			...
			if (state == 3) {
				...
				if (opcode == Opcodes.IF_ACMPNE) {
					...
					// and append this:
          // && info.loenwind.waterhooks.Hooks.allowFormWaterSourceBlock(worldIn, pos)
					mv.visitVarInsn(Opcodes.ALOAD, 1);
					mv.visitVarInsn(Opcodes.ALOAD, 2);
          mv.visitMethodInsn(Opcodes.INVOKESTATIC, "info/loenwind/waterhooks/Hooks", "allowFormWaterSourceBlock",
