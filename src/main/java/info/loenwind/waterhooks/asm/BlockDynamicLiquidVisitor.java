	...
	private static class HookMethodVisitor extends MethodVisitor {
      ...
      7->8 mv.visitJumpInsn(IF_ACMPNE, l23);
		...
		public void visitFieldInsn(int opcode, String owner, String name, String desc) {
			...
			if (state == 1) {
				if (opcode == Opcodes.GETFIELD && "net/minecraft/block/BlockDynamicLiquid".equals(owner) && "field_149815_a".equals(name)) {
					state ++;
				} else {
					state = 0;
				}
		...
		public void visitJumpInsn(int opcode, Label label) {
			...
			if (state == 3) {
				...
				if (opcode == Opcodes.IF_ACMPNE) {
					...
					mv.visitMethodInsn(Opcodes.INVOKESTATIC, "info/loenwind/waterhooks/Hooks", "allowFormWaterSourceBlock", "(Lnet/minecraft/world/World;III)Z", false);
